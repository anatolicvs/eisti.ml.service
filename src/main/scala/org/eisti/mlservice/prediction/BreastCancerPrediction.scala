...
@Service
class BreastCancerPrediction(@Autowired val spark:SparkSession ) {

   @Autowired
   var environment: Environment = null

  @Value("#{environment['credit.risk.data.file'] ?: '/Users/aytacozkan/data-bi/data/wbcd.csv'}")
  val dataUri: String = null

   def parseRDD(rdd:RDD[String]): RDD[Array[Double]] = {

     rdd.map(_.split(","))
       .filter(_(6) != "?").map(_.drop(1))
  ...
  def predictCancerRisk() : Map[String,Any] = {

    import spark.implicits._
    import spark._

    val obs = parseRDD(spark.sparkContext.textFile(dataUri)).collect().map(parseObs)

    val obsDF = spark.createDataFrame(obs).cache()
